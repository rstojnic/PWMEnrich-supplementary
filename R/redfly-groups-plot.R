...
load("../data/res-redfly-groups.RData")

#' Calc ranks for groups
#'
#' @param res a list of results res[[1]] for the first motif
#' @param expected.tfs what to expect
#' @param bg if to use background or not
calcRanks = function(res, expected.tfs, bg){
	ranks = list()
	for(i in 1:length(res)){
		r = groupReport(res[[i]], bg=bg)
		targets = r$target
		rr = r$rank
...
num.motifs = ncol(res$logn[[1]]$sequence.nobg)

# calculated the expected motifs for each group... 
load("../data/redfly/crms-v3_2014-10-13-intersectPWMEnrich.RData")
crms.tfbs = lapply(crms.by.tf, function(x) unique(unlist(crms.tfbs[x])))
...
ranks$clover = calcRanks(res$clover, crms.tfbs, FALSE)

# plot everything 
pdf("figs/redfly-groups.pdf", width=12, height=6)
par(mfrow=c(1,2))
col = brewer.pal(9, "Set1")

lwd = 2
# without BG
plotPrcRanks(ranks$count5, num.motifs, xlim=c(0, 0.5), col=col[5], rank.pch=7, lwd=lwd, main="Without background correction")
plotPrcRanks(ranks$count4, num.motifs, col=col[4], add=TRUE, rank.pch=6, lwd=lwd)
plotPrcRanks(ranks$count1e4, num.motifs, col=col[3], add=TRUE, rank.pch=5, lwd=lwd)
plotPrcRanks(ranks$count1e3, num.motifs, col=col[2], add=TRUE, rank.pch=4, lwd=lwd)
plotPrcRanks(ranks$count1e2, num.motifs, col=col[1], add=TRUE, rank.pch=3, lwd=lwd)
plotPrcRanks(ranks$clover, num.motifs, col="grey", add=TRUE, rank.pch=2, lwd=lwd)
plotPrcRanks(ranks$affinity, num.motifs, col="black", add=TRUE, rank.pch=1, lwd=lwd)
legend("topright", lty=rep(1, 7), col=c("black", "grey", col[1:5]), lwd=2, pch=1:7,
	legend=c("Affinity", "Clover", "P-value=1e-2", "P-value=1e-3", "P-value=1e-4",
	         "Cuttof=4", "Cutoff=5"))

# with BG
plotPrcRanks(ranks$cutoff5, num.motifs, col=col[5],  xlim=c(0, 0.5), rank.pch=6, lwd=lwd, main="With background correction")
